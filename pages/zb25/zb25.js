...
Page({
  ...
  save: function () {
    wx.setStorageSync('search_result_zb25', this.data.result);
    wx.setStorageSync('search_hisory_zb25', this.data.history)
  },
  load: function () {

    var srcData = wx.request({
      url:'https://localhost/js/ioapp/Index/zb25',
      success:(res)=>{
    ...
    var result = wx.getStorageSync('search_result_zb25');
    if (result) {
      var itemCount = result.length
      this.setData({ result: result, itemCount: itemCount })
    }
    var history = wx.getStorageSync('search_hisory_zb25')
    if (history) {
